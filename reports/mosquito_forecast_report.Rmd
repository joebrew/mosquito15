...
temp$mosquitos <- temp$n

ggplot(boundary, aes(long, lat)) +
  geom_polygon(color = 'darkgrey', fill = 'grey', alpha = 0.3) + 
  coord_equal() +
  theme_opts +
  geom_point(data = temp, aes(x = long, y = lat, size = mosquitos), pch = 16,
...
The most recent trap collection was on `r format(recent, '%B %d, %Y')`. The `r ts$n[which(ts$date == recent)]` mosquitos trapped (approximately `r round(ts$n[which(ts$date == recent)] / ts$n_traps[which(ts$date == recent)])` per trap) is at the `r num` percentile of all historical trappings.
...
g + 
  ...
  scale_x_log10() +
  xlab('Mosquitos per trap (weekly) (log scale)') +
  theme_bw() +
  geom_vline(xintercept = val, lwd = 2, alpha = 0.5)
```

Though numbers have declined over the last few weeks, recent weather conditions suggest a rise in the mosquito population through the middle of July.
...
Numbers so far this summer have been relatively low.  However, it's worth noting that peak mosquito population is usually reached later in the summer.
...
Vectors of diseases of concern appear relatively evenly distributed throughout the county.
...
Over the next two weeks, we predict the mosquito population to grow slightly, with the greatest growth ocurring in the south of the county (towards Micanopy).
...
# right$trap <- as.numeric(as.character(right$trap))
# temp <- left_join(x = left, y = right)
# temp$mosquitos <- temp$predicted
# temp <- temp[which(temp$date <= '2015-07-20'),]
# 
...
g + geom_point(color = 'blue', alpha = 0.5) +
  labs(x = 'Mosquitos predicted', y = 'Actual mosquitos', title = 'Model performance') +
  theme_bw() +
  geom_smooth()
```
